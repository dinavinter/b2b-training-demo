...
def generate_root_ca(out_cert_file=None, out_key_file=None, serial=1000, common_name="CA"):
    ...
    cert.get_subject().O = "Bla-%s" % os.urandom(4).encode("hex") # Just so we never have multiple root CAs with the same subject.
    ...
    exts = [
        ...
        crypto.X509Extension("keyUsage", True, "digitalSignature,keyCertSign"),
...
def _generate_ssl_cert(ca_cert, dns_names=None, ip_addrs=None, common_name=None, serial=1000, 
    ...
    exts = [
        ...
        crypto.X509Extension("authorityKeyIdentifier", False, "keyid", issuer=ca_cert.cert),
        crypto.X509Extension("subjectKeyIdentifier", False, "hash", subject=cert),
    ]
    if san_str:
        exts.append(crypto.X509Extension("subjectAltName", False, san_str))
    cert.add_extensions(exts)    
...
if __name__ == "__main__":
    #ca = generate_root_ca("test_rootCA.crt", "test_rootCA.key", serial=0x12345, common_name="TestRootCA")
